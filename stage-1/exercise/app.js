...
addListener('#form-unanswered', "submit",  function (e) {
  ...
  fetch ('https://api.stackexchange.com/2.2/questions/unanswered?order=desc&sort=activity&site=stackoverflow&tagged=' + tags)

  
});


addListener('#form-answerers', function (e) {
  e.preventDefault();
  var form = e.target;
  var tag  = form.querySelector('input[name=tags]').value;
  fetch ('https://api.stackexchange.com/2.2/tags/' + tag + '/top-answerers/all_time?site=stackoverflow')
});
